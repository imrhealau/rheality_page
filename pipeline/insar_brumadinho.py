#!/usr/bin/env python3
"""Real Sentinel-1 InSAR hindcast over the Corrego do Feijao Dam I, Brumadinho.

Rebuilds the deformation record of the tailings dam that failed on
25 Jan 2019: builds a small-baseline (SBAS) network of Sentinel-1 burst
interferograms ending the day before failure, has ASF HyP3 process them in
the cloud, then inverts the stack into a line-of-sight displacement time
series and a velocity, referenced to a stable point.

Data: Copernicus Sentinel-1 (ESA), processed by ASF HyP3 (ISCE burst InSAR).
Needs a free NASA Earthdata login in ~/.netrc.

Usage:
  .venv/bin/python -u insar_brumadinho.py submit    # queue the interferograms
  .venv/bin/python -u insar_brumadinho.py status    # check job progress
  .venv/bin/python -u insar_brumadinho.py process   # download + invert -> JSON
"""
import os, sys, json, zipfile, glob, math
from datetime import datetime, timezone

HERE = os.path.dirname(__file__)
JOBS = os.path.join(HERE, "data", "hyp3_jobs_brumadinho.json")
INSAR_DIR = os.path.join(HERE, "insar_brumadinho")
OUT = os.path.join(HERE, "data", "brumadinho_insar.json")

# Corrego do Feijao Dam I, Brumadinho, which failed 25 Jan 2019.
# Descending track 53, burst 053_112890_IW1. Window ends the day before failure.
DAM = {"lat": -20.1189, "lon": -44.1197}
TRACK = 53
DATE_START, DATE_END = "2017-06-01", "2019-01-24"
LOOKS = "10x2"                 # ~25 m pixels
WAVELENGTH = 0.055465          # Sentinel-1 C-band, metres
BATCH_NAME = "brumadinho-dam-insar"
# Small AOI around the dam for the inversion (W, S, E, N)
AOI = [-44.16, -20.15, -44.08, -20.09]
...
def build_network():
    ...
    scenes = sorted(((x.properties["sceneName"], x.properties["startTime"][:10]) for x in r
                     if "_VV_" in x.properties["sceneName"]),
                    key=lambda s: s[1])
    seen, epochs = set(), []
...
def cmd_process():
    ...
    out = {
        "generated_utc": now(),
        "target": {"name": "Corrego do Feijao tailings dam, Brumadinho", **DAM, "track": TRACK, "burst": "053_112890_IW1", "failure_date": "2019-01-25"},
